...
| T10 | FeedbackButton과 입력 버튼 겹침 현상(디바이스마다 유연한 대응이 필요하게) |
| T11 | 모바일 탭바 일관성                                                        | 게스트/로그인 전환 시 탭바 항목 변화 검증 필요             | P3           |
...
| P2       | Feedback 이미지 업로드         | 중간     | 2026.03.25 진행 중   |
| P3       | URL 교재 기능 (Cheerio 크롤링) | 중간     |                      |
| P4       | 학습 후 자동 퀴즈              | 중간     |                      |
| P5       | 스케줄 복습 알림 (에빙하우스)  | 어려움   |                      |
...
## 🆕 Feedback 이미지 업로드 기능 (v0.2 P2)

### 2026.03.25 완료 사항

- ✅ 책갈피 스타일 UI — "FEEDBACK" 텍스트 세로 표시, 오른쪽 상단 모서리 배치
- ✅ 모달 헤더에 "뭐 불편한 건 없으신지...?" 안내 문구 추가
- ✅ 기존 FeedbackButton 호버 툴팁 제거

### 2026.03.26 예정 — 파일 업로드 기능

| 항목 | 설정 | 비고 |
| ---- | ---- | ---- |
| **이미지 형식** | JPG, PNG, WebP만 지원 | GIF/SVG는 나중에 추가 검토 |
| **여러 개 업로드** | 최대 3개 | 용량 초과 시 경고 |
| **파일 크기 제한** | 5MB / 파일 | 총 15MB 제한 |
| **저장 방식** | Supabase Storage | `feedback/images/{fileId}` 경로 |
| **DB 스키마** | feedback 테이블에 `image_urls` JSONB 컬럼 추가 | 또는 별도 feedback_images 테이블 |
| **클라이언트 UI** | 파일 입력 버튼 + 이미지 프리뷰 스트립 | 썸네일 표시 및 제거 버튼 포함 |

### 결정 대기 사항

1. **DB 스키마**
   - A안: `feedback.image_urls` (JSONB array)
   - B안: 별도 `feedback_images` 테이블 + FK

2. **이미지 공유 범위**
   - user_id = NULL (게스트) 피드백의 이미지도 저장할지?
   - 개인정보 보호를 위한 정책 필요
