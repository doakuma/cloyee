...
> 최종 업데이트: 2026-03-24 | 코드베이스 기반 실제 동작 기준
...
| 섹션 | 내용 |
|------|------|
| Hero | 앱 소개 + `LandingCTA` (Google 로그인 or 인앱 경고 배너) |
...
> **인앱 브라우저**: `LandingCTA`가 UA 감지 → 인앱이면 경고 배너 표시 (iOS: 이메일 링크 유도 / Android: Chrome에서 열기 버튼)
...
| 환경 | 동작 |
|------|------|
| iOS 인앱 (카카오톡/인스타 등) | Magic Link 탭 자동 전환 + 경고 배너 ("Safari에서 열어주세요") |
| Android 인앱 | 경고 배너 + "Chrome에서 열기" 버튼 (Intent URL) |
| 인앱 공통 | 소셜(Google/GitHub) 버튼 숨김 처리 |
...
| `/admin/feedback` | 전체 피드백 목록 (카테고리 뱃지, 내용, 사용자 ID, 날짜) |
...
모든 페이지 우측 하단에 고정 표시 (`/admin` 제외).

| 요소 | 설명 |
|------|------|
| 버튼 | 원형 FAB, hover 시 툴팁 "뭐 불편한 건 없으신지...?" |
| 클릭 | Dialog 모달 오픈 |
| 카테고리 선택 | 버그 신고 🐛 / 개선 제안 💡 / 기타 💬 |
| 전송 | Supabase `feedback` 테이블 insert → Sonner toast 확인 |
